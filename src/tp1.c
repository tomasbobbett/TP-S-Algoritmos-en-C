...
struct tp1 {
	struct pokemon *
		pokemones_nombres; // Arrglo dinamico de pokemones ordenados por NOMBRE
	struct pokemon
		*pokemones; // arreglo dinámico de pokemones ordenados por ID
	size_t cantidad,
		capacidad; // cantidad de pokemones almacenados y capacidad del arreglo
};

// -----------------FUNCIONES AUXILIARES TO-DO(SACAR DEL TP0 A ARCHIVO SOLO DE
// FUNC AUXILIARES)-----------------
...
static struct pokemon *busqueda_binaria_pokemon(tp1_t *tp, const char *nombre,
						int inicio, int final)
{
	...
	if (inicio > final)
		return NULL;
	if (inicio == final) {
		if (tp->pokemones_nombres[inicio].nombre &&
		    strcmp(tp->pokemones_nombres[inicio].nombre, nombre) == 0) {
...
static struct pokemon *busqueda_binaria_id(tp1_t *tp, int id, int inicio,
					   int final)
{
	if (!tp)
		return NULL;
...
static bool tp1_redimensionar(tp1_t *tp)
{
	...
	size_t nueva_capacidad = tp->capacidad ? tp->capacidad * 2 :
						 CAPACIDAD_INICIAL_TP1;

	struct pokemon *nuevo_array =
		malloc(nueva_capacidad * sizeof(struct pokemon));
	struct pokemon *nuevo_array_nombres =
		malloc(nueva_capacidad * sizeof(struct pokemon));
...
static bool insertar_nombre_ordenado(struct tp1 *tp, struct pokemon p)
{
	...
	size_t i = tp->cantidad;
	while (i > 0 &&
	       strcmp(tp->pokemones_nombres[i - 1].nombre, p.nombre) > 0) {
...
// Lee una línea completa de un archivo, sin límite fijo
// Devuelve un malloc() con la línea (hay que free()), o NULL si EOF/error
static char *leer_linea(FILE *f)
{
	size_t cap = 128;
	size_t len = 0;
	char *buf = malloc(cap);
	...
	while ((c = fgetc(f)) != EOF) {
		...
		if (len + 1 >= cap) { // +1 para '\0'
			size_t nueva_cap = cap * 2;
			char *tmp = realloc(buf, nueva_cap);
			if (!tmp) {
				free(buf);
				return NULL;
			}
...
tp1_t *tp1_leer_archivo(const char *nombre)
{
	...
	char *linea;
	while ((linea = leer_linea(file)) != NULL) {
		int id, ataque, defensa, velocidad;
		char nombre_poke[25656], tipo_str[25656];
		int n = sscanf(linea, "%d,%[^,],%[^,],%d,%d,%d", &id,
			       nombre_poke, tipo_str, &ataque, &defensa,
			       &velocidad);
		free(linea);
		if (n < 6)
			continue;

		if (tp1_buscar_id(tp, id) != NULL) {
			continue; // id repetido, lo salto
		}
		char tipo = parsear_string_tipo(tipo_str);
		if (tipo == -1) {
			continue;
		}

		char *nombre_copia = mi_strdup(nombre_poke);
		if (!nombre_copia) {
			tp1_destruir(tp);
			fclose(file);
			return NULL;
...
tp1_t *tp1_union(tp1_t *un_tp, tp1_t *otro_tp)
{
	...
	while (i < un_tp->cantidad && j < otro_tp->cantidad) {
		if (un_tp->pokemones[i].id < otro_tp->pokemones[j].id) {
			...
			res->pokemones[k].nombre =
				mi_strdup(un_tp->pokemones[i].nombre);
			i++;
		} else if (un_tp->pokemones[i].id > otro_tp->pokemones[j].id) {
			res->pokemones[k] = otro_tp->pokemones[j];
			res->pokemones[k].nombre =
				mi_strdup(otro_tp->pokemones[j].nombre);
			j++;
		} else {
			res->pokemones[k] = un_tp->pokemones[i];
			res->pokemones[k].nombre =
...
{
	if (!tp || tp->cantidad == 0 || !tp->pokemones_nombres ||
	    nombre == NULL)
		return NULL;
...
{
	if (!tp || tp->cantidad == 0 || !tp->pokemones)
		return NULL;
...
size_t tp1_con_cada_pokemon(tp1_t *un_tp, bool (*f)(struct pokemon *, void *),
			    void *extra)
{
	if (!un_tp || !f || un_tp->cantidad == 0)
		return 0;

	size_t cont = 0;

	for (size_t i = 0; i < un_tp->cantidad; i++) {
		cont++; // Contamos este Pokémon
		if (!f(&un_tp->pokemones[i], extra))
			break; // Detener iteración si f devuelve false
	}
	...
	if (cap > 0) {
		...
		if (unicos) {
			for (size_t i = 0; i < n; i++) {
				if (tp1->pokemones &&
					...
					if (!seen)
						unicos[unicos_count++] = name;
				}
				if (tp1->pokemones_nombres &&
				    tp1->pokemones_nombres[i].nombre) {
					char *name =
						tp1->pokemones_nombres[i].nombre;
					bool seen = false;
					for (size_t j = 0; j < unicos_count;
			...
			for (size_t i = 0; i < unicos_count; i++)
				free(unicos[i]);
			free(unicos);
		} else {
       ...
       */
			if (tp1->pokemones) {
				...
				for (size_t i = 0; i < n; i++)
					free(tp1->pokemones_nombres[i].nombre);
			}
		}
	}
