...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(modo, NOMBRE) == 0) {
        ...
        if (res) imprimir_pokemon(res);
        else printf("No se encontró pokemon con nombre '%s'\n", valor);
    } else if (strcmp(modo, ID) == 0) {
        int id = atoi(valor);
        res = buscar_por_id(lista, id);
        if (res) imprimir_pokemon(res);
        else printf("No se encontró pokemon con id %d\n", id);
    } else {
        imprimir_uso(argv[0]);
    }

    lista_destruir(lista);     // la lista no libera pokemones, sólo los punteros
    tp1_destruir(pokemones);  
    return 0;
}
